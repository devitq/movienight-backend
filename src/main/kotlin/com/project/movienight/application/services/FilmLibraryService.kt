...
class FilmLibraryService(
    ...
    override fun addFilm(command: AddFilmToLibraryCommand): FilmLibrary {
        ...
        if (existingEntry != null) {
            ...
            val saved = filmLibraryRepository.save(
                existingEntry.copy(
                    isViewed = false,
                    watchedAt = null,
                )
        ...
        businessMetricsService.recordLibraryEvent()
        log.info("Film added to library: userId={}, filmId={}, entryId={}", command.userId, command.filmId, saved.id)
        return saved
    }
    ...
    override fun markViewed(command: MarkFilmViewedCommand): FilmLibrary {
        ...
        val saved = if (existingEntry == null) {
            ...
            log.debug("Updating existing entry: entryId={}, was viewed={}", existingEntry.id, existingEntry.isViewed)
            filmLibraryRepository.save(
                existingEntry.copy(
                    isViewed = true,
                    watchedAt = watchedAt,
        ...
        businessMetricsService.recordLibraryEvent()
        log.info("Film marked as viewed: userId={}, filmId={}, entryId={}", command.userId, command.filmId, saved.id)
        return saved
    }

    override fun getLibrary(query: GetFilmLibraryQuery): FilmLibrary {
        log.debug("Getting library for user: {}", query.userId)
        val library = findByUserId(query.userId)
    ...
    private fun findByUserAndFilmId(userId: UUID, filmId: UUID): FilmLibrary? {
        return filmLibraryRepository.findAll().firstOrNull { it.userId == userId && it.filmId == filmId }
    }
}
